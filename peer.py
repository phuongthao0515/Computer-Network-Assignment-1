...
def getAllConnectedChannel():
    channel_list = []
    print("CH", client.hosts)
    if client.channels:
        for key, host_key in client.channels.item():
            channel_list.append( {"channel_name": key, "peer-server-ip" : host_key[1], "peer_server_port" : host_key[2]} )
    return channel_list
